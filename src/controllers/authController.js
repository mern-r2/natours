...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    // auth user based on tokens via cookies
  ...
  // Verify token returning { id, iat, exp } obj
  const decoded = await promisify(jwt.verify)(token, env.jwtSecret);

  // Verify if token is still valid for the user
...
exports.isLoggedIn = async (req, res, next) => {
  ...
  try {
    // verify token
    const decoded = await promisify(jwt.verify)(req.cookies.jwt, env.jwtSecret);
    ...
    // THERE IS A LOGGED IN USER - make it accessible to pug templates
    res.locals.user = currentUser;
    return next();
  } catch (err) {
    return next();
